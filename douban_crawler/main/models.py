...
class Movies(models.Model):
    ...
    grade = models.FloatField(default=0.0)#评分
    num_evaluate = models.BigIntegerField(default=0)#评价数
    sketch = models.CharField(max_length=2000)#简述
    other_info = models.CharField(max_length=2000)#相关信息
    # 打印
    def __str__(self):
        return (f"电影: {self.name} (排名: {self.rank}, 评分: {self.grade}, "
                f"评价数: {self.num_evaluate}, 国外名称: {self.name_foreign})")